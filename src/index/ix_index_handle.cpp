...
void IxNodeHandle::erase_pair(int pos) {
    ...
    memmove( key_slot, key_slot+len, mv_size*len );
...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    int parent_insert_pos = parent_node->upper_bound(key);
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    if( node->get_page_no() == file_hdr_->root_page_ ){
        ...
        IxNodeHandle *neighbor = fetch_node( parent->get_rid( index+(index?-1:1) )->page_no );
